...
#taking job_id from consoletext,timestamp and pass/fail
...
#----------------------------------------------------------------------------------------------------------------->
#help for cs team to fetch detail
...
if jbuild == abuild:
	...
	print "This is not official proposal build but might be CS run or testrun.Hence inserting update ID in build column for DB operation"
        try:
                cursor.execute("insert into %s VALUES('%s', '%s', '%s', '%s', '%s', '%s', '%s','%s','%s')" %(db_stage,jid, jname, abuild, jstime, jetime, jstatus, jrerun,jetime-jstime,db_name))
                db.commit()
        except MySQLdb.Error, e:
                 print(e)
